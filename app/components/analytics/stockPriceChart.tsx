...
export default function StockPriceChart({ ticker }: { ticker: string }) {
  ...
  return (
    <div className="stokr-card min-w-0 p-4 sm:p-5">
            ...
            return (
              <button
                ...
                className={`min-w-0 rounded-md px-3 py-2 text-sm font-semibold transition sm:px-4 ${isActive
                  ? "border border-[#F0EDE6] bg-[#F0EDE6] text-[#0C0C0C]"
                  : "border border-[#2E2D2A] bg-transparent font-mono text-[#9A9690] hover:bg-[#161616] hover:text-[#F0EDE6]"
                  }`}
              >
                {range}
              </button>
      ...
      {loading && (
        <div className="flex h-[320px] items-center justify-center border border-[#222120] bg-[#0C0C0C]">
      ...
      {!loading && error && (
        <div className="flex h-[320px] items-center justify-center border border-[#222120] bg-[#0C0C0C] p-5">
          <div className="text-center">
      ...
      {!loading && !error && chartResponse && (
        <>
          ...
          {chartResponse.isFallback ? (
            <div className="flex h-[320px] items-center justify-center border border-[#222120] bg-[#0C0C0C] p-6 text-center">
              ...
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="priceFill" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#D63C2F" stopOpacity={0.35} />
                    <stop offset="95%" stopColor="#D63C2F" stopOpacity={0.03} />
                  </linearGradient>
                </defs>

                <CartesianGrid
                  stroke="rgba(240, 237, 230, 0.10)"
                  vertical={false}
                />
                <XAxis
                  dataKey="label"
                  tick={{ fill: "#9A9690", fontSize: 12 }}
                  axisLine={false}
                  tickLine={false}
                />
                <YAxis
